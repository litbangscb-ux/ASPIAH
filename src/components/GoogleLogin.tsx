...
export default function GoogleLogin({ onLogin, availableUsers }: GoogleLoginProps) {
  ...
  const triggerGoogleOAuthSimulation = (user: AppUser) => {
    setIsLoading(true);
    setLoadingStep('Membuka Saluran Google OAuth 2.0...');
      ...
      {isLoading ? (
        ...
        <div>
          ...
          <form onSubmit={handleCustomLoginSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl text-xs font-sans flex items-center justify-center gap-2 shadow-md hover:shadow-lg hover:shadow-blue-100 transition-all cursor-pointer active:scale-95 block"
            >
              {/* Standard multi colored authentic simulation element */}
              <div className="w-4 h-4 rounded-full bg-white flex items-center justify-center shadow-xs text-[10px] font-bold text-red-500 select-none mr-1 inline-flex">
                G
              </div>
